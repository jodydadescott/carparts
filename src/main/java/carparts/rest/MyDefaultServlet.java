 ...
 * OPTION and TRACE methods for the context. The following initParameters are
 * supported, these can be set either on the servlet itself or as ServletContext
 * initParameters with a prefix of org.eclipse.jetty.servlet.Default. :
 * 
 * <pre>
 ...
 *
 */
public class MyDefaultServlet extends HttpServlet implements ResourceFactory, WelcomeFactory {
	public static final String CONTEXT_INIT = "org.eclipse.jetty.servlet.Default.";

	private static final Logger LOG = Log.getLogger(MyDefaultServlet.class);
	...
	/* ------------------------------------------------------------ */
	public MyDefaultServlet(ResourceService resourceService) {
	...
	public void init() throws UnavailableException {
		...
		_resourceService.setPrecompressedFormats(
				parsePrecompressedFormats(getInitParameter("precompressed"), getInitBoolean("gzip", false)));
		...
		try {
			if (_cache == null && (max_cached_files != -2 || max_cache_size != -2 || max_cached_file_size != -2)) {
				_cache = new CachedContentFactory(null, this, _mimeTypes, _useFileMappedBuffer,
						_resourceService.isEtags(), _resourceService.getPrecompressedFormats());
				if (max_cache_size >= 0)
	 ...
	 * likely that this method needs to be overwritten to unwrap the ServletContext
	 * facade until we reach the original jetty's ContextHandler.
	 * 
	...
	/* ------------------------------------------------------------ */
	/**
	 * get Resource to serve. Map a path to a resource. The default implementation
	 * calls HttpContext.getResource but derived servlets may provide their own
	 * mapping.
	 * 
	...
	public String getWelcomeFile(String pathInContext) {
		...
		for (int i = 0; i < _welcomes.length; i++) {
			...
			if ((_welcomeServlets || _welcomeExactServlets) && welcome_servlet == null) {
				...
				if (entry != null && entry.getResource() != _defaultHolder && (_welcomeServlets
						|| (_welcomeExactServlets && entry.getPathSpec().getDeclaration().equals(welcome_in_context))))
